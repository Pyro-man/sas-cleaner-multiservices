# Script pour ajouter les liens Transport et Déménagement aux menus des autres pages - VERSION FINALE
...
function Process-File {
    ...
    if ($content -match $pattern) {
        ...
        if ($content -match $pattern2) {
            $newContent = $content -replace $pattern2, $linksToAdd + $pattern2
            Set-Content $filePath $newContent -Encoding UTF8
